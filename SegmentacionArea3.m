function SegmentacionArea3
% Para coger los ficheros de su ubicación
cd('C:\Users\USUARIO\Documents\Estudios\Universidad\4to\4to Segundo Cuatri\TFG\TFG\Raman\2025-03-24 Análisis 3º experimento\HeLa TIFF_average\Cell\');
...
for i=1:nI
 ...
 %% Células
 Celula_contraste=imadjust(Celula, [0.137 0.468], [0 1]); %8-bit= 30-120
 ...
 %figure, histogram(vector_pixeles_eti, 50), title('Histograma de Tamaños de Áreas'); xlabel('Tamaño de área (píxeles)'); ylabel('Frecuencia')
 ...
 %figure, histogram(vector_pixeles_etif, 15), title('Histograma de Tamaños de Células'); xlabel('Tamaño de área (píxeles)'); ylabel('Frecuencia')


 binary_final_cel=imbinarize(etiquetas_final_cel); 
 final_cel_255=binary_final_cel*255;

 ruta = 'C:\Users\USUARIO\Documents\Estudios\Universidad\4to\4to Segundo Cuatri\TFG\TFG\Raman\2025-03-24 Análisis 3º experimento\Mascaras\Mascaras_Celulas';
 filename = sprintf('Mascara_Cel%s.tif', nombre_exp(i));
 filepath = fullfile(ruta, filename);
 imwrite(final_cel_255, filepath);

 %% LDs
 cd('C:\Users\USUARIO\Documents\Estudios\Universidad\4to\4to Segundo Cuatri\TFG\TFG\Raman\2025-03-24 Análisis 3º experimento\HeLa TIFF_average\LD\');
 L=dir('*.tif');
 LD=imread(L(i).name);
 LD_contraste=imadjust(LD, [0.341 1], [0 1]); %8-bit= 84-255
 %figure, imshow(LD_contraste), set(gcf, 'Name', 'ventana LDs y autoF')

 LD_cel= LD_contraste .* uint8(binary_final_cel); %aplicar la máscara a los LDs
 %figure, imshow(LD_cel), set(gcf, 'Name', 'esta imagen no debería ser binaria')
 LD_aver = imfilter(LD_cel, aver, 'same');
 ...
 %figure, imshow(etiquetas), set(gcf, 'Name', 'Etiquetas sin filtrar, full')
 ...
 %figure, histogram(vector_pixeles_etiLD, 100), title('Histograma de Tamaños de Áreas'); xlabel('Tamaño de área (píxeles)'); ylabel('Frecuencia')
 ...
 ruta2= 'C:\Users\USUARIO\Documents\Estudios\Universidad\4to\4to Segundo Cuatri\TFG\TFG\Raman\2025-03-24 Análisis 3º experimento\Mascaras\Mascaras_LD';

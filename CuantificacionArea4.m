...
Size_cel=zeros(size(unique(etiquetas_final_cel),1),1);
 ...
 for b=1:t
  ...
  %MÉTODO A PARA CALCULAR EL TAMAÑO DE LOS LD PARA EL HISTO
%Size_cel=Size_cel(Size_cel>0);  
%tabla_size_cell{i,1}=Size_cel;
%Size_LD=Size_LD(Size_LD>0);
%tabla_size_LD{i,1}=Size_LD;
...
archivo = 'AreaporAreaporCel4.xlsx';
...
ruta = 'C:\Users\USUARIO\Documents\Estudios\Universidad\4to\4to Segundo Cuatri\TFG\TFG\Raman\2025-05-09 Análisis completo\Areas';
archivo = 'MediaAreaAYB4.xlsx';
writetable(T2, fullfile(ruta, archivo));

figure, bar(LDsarea), title('CSP effect on LD generation'), xlabel('Experimental condition'), ylabel('LD area/Cell area'), set(gcf, 'Name', 'Area result')
hold on
x = 1:length(Experimento);
xticks(x);
...
ruta = 'C:\Users\USUARIO\Documents\Estudios\Universidad\4to\4to Segundo Cuatri\TFG\TFG\Raman\2025-05-09 Análisis completo\Areas';
archivo = 'MediaArea4.xlsx';
writetable(T3, fullfile(ruta, archivo));

figure, bar(LDsareaAYB), title('CSP effect on LD generation'), xlabel('Experimental condition'), ylabel('LD area/Cell area'), set(gcf, 'Name', 'Area result')
hold on

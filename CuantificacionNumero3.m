function CuantificacionNumero3
...
%comparacion wt y ko (unificacion a+b)
...
vector_WT_control_n=tabla_numero_final.Vectores_numeroporcel{1}'; %como de los controles no tenemos 2 espectros, tenemos que incorporarlos manualmente
media_nLD_por_celula_WT_control=mean(vector_WT_control_n);
sd_nLD_por_celula_WT_control=std(vector_WT_control_n);
LDsnAYB(1,1)=media_nLD_por_celula_WT_control; %guarda el resultado para esta imagen
desviacion_nAYB(1,1)=sd_nLD_por_celula_WT_control/ sqrt(length(vector_WT_control_n)); %error estándar

vector_KO_control_n=tabla_numero_final.Vectores_numeroporcel{2}'; %como de los controles no tenemos 2 espectros, tenemos que incorporarlos manualmente
media_nLD_por_celula_KO_control=mean(vector_KO_control_n);
sd_nLD_por_celula_KO_control=std(vector_KO_control_n);
LDsnAYB(2,1)=media_nLD_por_celula_KO_control; %guarda el resultado para esta imagen
desviacion_nAYB(2,1)=sd_nLD_por_celula_KO_control/ sqrt(length(vector_KO_control_n)); %error estándar

    %unificacion de vectores
    for i = 3:2:num_vectores1
        vector_AYB = [tabla_numero_final.Vectores_numeroporcel{i}', tabla_numero_final.Vectores_numeroporcel{i+1}']; %esto también se podría guardar en excel
...
figure, bar(LDsnAYB(1:2)), title('CSP effect on LD generation'), xlabel('Experimental condition'), ylabel('LD area/Cell area'), set(gcf, 'Name', 'Area result')
hold on
x = 1:length(ExperimentoAYB);
xticks(x);
xticklabels({'WT Control','KO Control'});
errorbar(x, LDsnAYB(1:2), desviacion_nAYB(1:2), 'k', 'LineStyle', 'none', 'LineWidth', 0.5);
hold off
